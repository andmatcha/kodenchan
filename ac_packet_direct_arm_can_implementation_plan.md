...
- motor CAN output:
  ...
  - `0x208`: `control_byte` bit0..7 を byte0..7 へ展開した補助フラグ
...
3. RPM / angle feedback を含む `ArmState` を読む。
...
7. `arm_state` を追加し、CAN feedback の snapshot を管理する。
8. `arm_control` を追加し、normalized manual 入力と PID から `ArmMotorCommand` を作る。
9. `ac_direct_arm_service` を追加し、`app_poll()` から packet 取り込み、snapshot 更新、周期制御、CAN 送信を行う。
10. `app.c` を新 service 初期化 / poll に差し替え、旧 `can_bridge` 呼び出しを停止する。
11. 実機前に pack 関数と ACv6 parser のテストを追加し、想定 byte order を固定する。
...
if (control_period_elapsed()) {
  ...
  arm_state_update_from_feedback(&state);
...
- `control_byte` の bit 操作で `0x208` byte0..7 が変わることを確認する。
...
- timeout、範囲外値に対する安全動作が実装されている。
- parser / packer / normalization のテストがある。
