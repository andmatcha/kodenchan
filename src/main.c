...
#define SERVO_SPEED 1.0f      // 角度変化速度 (度/20ms)

volatile float servo_current_angle = 135.0f;  // 現在の角度 (初期値: 中央)
volatile uint8_t servo_command = 1;           // 0: 増加, 1: 停止, 2: 減少
...
// 角度を更新 (タイマー割り込みから呼ばれる)
void Servo_UpdateAngle(void) {
  if (servo_command == 0) {
    // 角度を増やす (270度方向)
    servo_current_angle += SERVO_SPEED;
    if (servo_current_angle > SERVO_ANGLE_MAX) {
      servo_current_angle = SERVO_ANGLE_MAX;
    }
...
int main(void)
{
  ...
  // TIM1を20ms周期で開始 (角度更新用)
  // TIM1設定: 32MHz / (1600) / (400) = 50Hz (20ms)
  htim1.Init.Prescaler = 1599;  // 32MHz / 1600 = 20kHz
  htim1.Init.Period = 399;      // 20kHz / 400 = 50Hz
